...
active[10] proctype Individu() {
	printf("Individu_%d, veut entrer !\n", _pid);
	chPresence!SIGNAL_PRESENCE;
	do
	:: chPortes?[SIGNAL_OUVERTES] -> ok_individu=1; printf("Individu_%d entre, merci.\n",_pid); break
	:: else -> printf("Individu_%d attend ...\n", _pid)
	od
}

active proctype Portes() {
	Fermees: 	chPortes?SIGNAL_OUVRIR; printf("Les portes s'ouvrent ...\n"); chPortes!SIGNAL_OUVERTES;
	Ouvertes:	chPortes?SIGNAL_FERMER; printf("Les portes se ferment ...\n"); chPortes!SIGNAL_FERMEES; goto Fermees;
}

/*S'il y a un Timer*/
active proctype Timer()  {
	Off: 	chTimer?SIGNAL_INIT; printf("J'initialise le compteur ...\n"); cpt = 0;
	On:	do
		:: chTimer?SIGNAL_INIT; printf("Je reinitialise le compteur ...\n"); cpt = 0
